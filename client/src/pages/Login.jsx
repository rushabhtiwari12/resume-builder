...
import api from '../app/configs/api'; // missing import
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await api.post(`/api/users/${state}`, formData); // fixed string interpolation syntax
      dispatch(login(data));
      localStorage.setItem('token', data.token);
      toast.success(data.message);
    } catch (error) {
      toast.error(error?.response?.data?.message || error.message); // added toast.error for consistency
    }
  };
